...
export const Dropzone: React.FC<DropzoneProps> = React.forwardRef<HTMLDivElement, DropzoneProps>((props, ref) => {
    ...
    // When the user drops/chooses a file
    ...
    const imageGridThumbs = files?.map((file: any, index) => {
        return (
            <div
                key={file.name}
                className={cn("relative bg-transparent border border-[--border] aspect-square h-full bg-center bg-no-repeat bg-contain rounded-md overflow-hidden", "col-span-1 row-span-1")}
                style={{
    ...
    return (
        <BasicField
            className={cn(DropzoneAnatomy.root(), rootClassName, className)}
            {...basicFieldProps}
        >
            <div
                className={cn(
                    "mb-2 cursor-pointer hover:text-[--text-color] flex flex-col items-center p-4 border border-[--border] rounded-md border-dashed text-[--muted] transition ease-in-out hover:border-[--text-color]",
                    {
                ...
                {files?.map((file: any, index) => {
                    ...
                    if (["image/jpeg", "image/png", "image/jpg", "image/webm"].includes(file.type)) {
                        Icon = <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                    strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5 flex-none">
                    ...
                    return (
                        <div key={file.name} className="w-full">
                            ...
                            <div
                                className="flex items-center justify-space-between relative p-1 hover:bg-gray-[--highlight] w-full overflow-hidden"
                            >
                                <div className="flex items-center gap-2 truncate w-full">
                                    {Icon}
                                    <p className="truncate max-w-[180px]">{file.name}</p>
                                    <p className="text-xs uppercase text-center font-semibold align-center">{humanFileSize(file.size)}</p>
                                </div>
                                <IconButton
                                    size="xs"
                                    intent="gray-basic"
